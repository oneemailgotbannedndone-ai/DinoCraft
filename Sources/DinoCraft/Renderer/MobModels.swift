...
enum PartRole {
    case body
    case head
    case leg(Float)      // walk-cycle phase offset
    case tail
    case segment(Int)
    case wing(Float)     // -1 left, +1 right
}
...
final class MobModelLibrary {
    ...
    private struct Builder {
        ...
        mutating func add(_ role: PartRole, _ pivot: SIMD3<Float>, _ boxes: [Box]) {
            var v: [ModelVertex] = []
            for b in boxes { ModelBuilder.box(&v, min: b.0, max: b.1, color: b.2, emissive: b.3 ? 1 : 0) }
            if let mesh = ModelMesh(device: device, vertices: v, label: "Mob part") {
                parts.append(MobPart(mesh: mesh, pivot: pivot, role: role))
            }
        }
    }
...
extension ModelRenderer {
    /// Creatures and their projectiles, animated from AI state.
    func encodeMobs(_ enc: MTLRenderCommandEncoder, session s: GameSession, camera: Camera, frame: inout FrameUniforms,
                    time: Double, library: MobModelLibrary) {
        guard !s.mobs.mobs.isEmpty || !s.mobs.projectiles.isEmpty else { return }
        begin(enc, &frame)
        let maxDist = Float(s.world.renderDistance * 16)
        for m in s.mobs.mobs {
            let rel = SIMD3<Float>(Float(m.position.x - camera.position.x), Float(m.position.y - camera.position.y),
                                   Float(m.position.z - camera.position.z))
            guard simd_length(rel) < maxDist else { continue }
            let light = s.world.light(at: m.position + DVec3(0, m.species.height * 0.6, 0))
            ...
            for part in library.parts(m.species.kind, variant: m.variant) {
                ...
                draw(enc, part.mesh, uniforms: &u)
            }
        }
        if let spit = library.spit {
            for p in s.mobs.projectiles {
                ...
                let model = MathUtil.translation(rel) * MathUtil.rotationY(Float(p.age * 9))
                var u = ModelUniforms(mvp: frame.viewProj * model, model: model, light: SIMD4(1, 1, 0, 0), tint: .zero, viewPos: SIMD4(rel, 1))
                draw(enc, spit, uniforms: &u)
            }
        }
    }
}
