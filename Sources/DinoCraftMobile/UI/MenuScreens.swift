...
final class MainMenuScreen: Screen {
    ...
    override func draw(_ ui: UIContext, _ e: GameEngine) {
        ...
        // Landscape phone layout: the title on the left, the buttons on the right.
        let leftX = W * 0.3, rightX = W * 0.72
        let top = max(30, H / 2 - 190)

        d.opacity = intro
        if let icon = e.iconTexture {
            let s: Float = 104
            let r = Rect(leftX - s / 2, top - (1 - intro) * 20 + bob * 0.5, s, s)
            d.fill(r.inset(10), Theme.amber.alpha(0.25), radius: 30, blur: 24)
            d.image(icon, r)
        }
        let titleY = top + 104 + bob
        d.outlinedText(Brand.title, x: leftX, y: titleY, size: Brand.title.count > 9 ? 64 : 76, fill: Color(hex: Brand.top), fillBottom: Color(hex: Brand.bottom),
                       outline: Color(hex: 0x3A2414), outlineWidth: 6, tracking: 0.005)
        d.text("A  PREHISTORIC  VOXEL  ADVENTURE", x: leftX, y: titleY + 92, size: 13, color: Theme.text.alpha(0.88), face: .display,
               align: .center, tracking: 0.18, shadow: Color(linear: 0, 0, 0, 0.7))
        d.opacity = 1

        let bw: Float = 340, bh: Float = 60, gap: Float = 16
        let items: [(String, String, ButtonStyle)] = [
            ("menu.sp", "Play", .primary),
            ("menu.settings", "Settings", .secondary),
            ("menu.credits", "Credits", .secondary),
        ]
        var y = H / 2 - (Float(items.count) * (bh + gap) - gap) / 2
        for (i, item) in items.enumerated() {
            let a = appear(0.25 + Double(i) * 0.07)
            d.opacity = a
            let r = Rect(rightX - bw / 2, y + (1 - a) * 26, bw, bh)
            if ui.button(item.0, item.1, r, style: item.2) {
                switch item.0 {
        ...
        d.text("Offline edition", x: W - 22, y: H - 34, size: 13, color: Theme.textMuted, align: .right,
    ...
    override func draw(_ ui: UIContext, _ e: GameEngine) {
        ...
        if ui.button("worlds.delete", "Delete", Rect(panel.x + 30 + (bw + 12) * 2 + 12, by, bw - 12, 52), style: .danger,
            ...
            e.pushScreen(ConfirmScreen(title: "Delete “\(w.name)”?",
                                       message: "This world will be deleted for good.",
